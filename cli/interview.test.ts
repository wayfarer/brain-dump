...
import { type Db, getNodeById, getRecentNodes, insertNode, openDb } from "./store.js";
...
function extracted(overrides: Partial<ExtractedNode> = {}): ExtractedNode {
  return { tag: "sudden loss", content: "I saw the dog", parentId: "", ...overrides };
}
...
describe("buildSystemPrompt", () => {
  ...
  it("prioritises search-matched nodes when recentInput is provided", async () => {
    ...
    expect(await buildSystemPrompt(db, openai, "life_story")).not.toContain("distant memory");
    expect(await buildSystemPrompt(db, openai, "life_story", "I was with my grandmother")).toContain("distant memory");
  });

  it("works with a null client (no embeddings) — degrades to FTS5", async () => {
    insertNode(db, makeNode({ id: "old", tag: "distant memory", content: "grandmother in the garden", capturedAt: 1 }));
    const prompt = await buildSystemPrompt(db, null, "life_story", "grandmother");
...
describe("persistNodes", () => {
  ...
  it("stores memoryDate and a valid granularity", () => {
    persistNodes(db, makeState(), [extracted({ memoryDate: "1994", memoryDateGranularity: "year" })], null);
    const stored = getRecentNodes(db, 1)[0];
  ...
  it("nulls an invalid granularity but keeps the date", () => {
    persistNodes(db, makeState(), [extracted({ memoryDate: "1990s", memoryDateGranularity: "invalid_value" })], null);
    const stored = getRecentNodes(db, 1)[0];
  ...
  it("computes child depth from a resolved parent", () => {
    ...
    persistNodes(db, makeState(), [extracted({ tag: "fear", content: "dark room", parentId: "parent-id" })], null);
  ...
  it("persists multiple nodes and points lastParentId at the last", () => {
    const state = makeState();
    persistNodes(db, state, [extracted({ tag: "a", content: "first" }), extracted({ tag: "b", content: "second" })], null);
  ...
  it("stores an embedding when one is provided", () => {
    expect(() => persistNodes(db, makeState(), [extracted()], makeMockEmbedding())).not.toThrow();
